...
服务端会汇总各启用测试点最近一次成功检查得到的出口 IP，并在 Xray 节点卡片中显示出口 IP、国家代码和公司名。不同测试点报告的 IP 不同时采用出现次数最多的 IP；票数相同则采用最近报告的 IP。归属信息由调度器通过 ipinfo.io 查询并缓存 7 天，查询失败会在 1 小时后重试，不影响检查结果上报。可选配置 `IPINFO_TOKEN` 使用账户额度，`IPINFO_TIMEOUT_SECONDS` 控制查询超时。
